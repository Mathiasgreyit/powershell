function Get-majoAzKeyVaultCertificate {
    ...
    .DESCRIPTION
        The purpose of this script is to simplify downloading and generate a pfx file from a Azure Keyvault. An Azure context must be present, ie Connect-AzAccount must be run before this command.
    .EXAMPLE
        PS C:\> Get-majoAzKeyVaultCertificate -CertificateName 'mycert' -VaultName myazurekeyvault
        Creates a pfx file including private key with a random password and save it as a random named pfx file in current directory. Outputs fullpath and password.
    .INPUTS
        [String]CertificateName, certificatename.
        [String]VaultName, name of Azure Keyvault where certificate is located.
    .OUTPUTS
        Outputs Filename and Password in non structured format.
    .NOTES
    ...
    param (
        ...
        [Parameter(Mandatory = $false,
            HelpMessage = 'Azure subscription where Keyvault exists',
            Position = 4)]
        [String]$Subscription,   
        [Parameter(Mandatory = $false,
            HelpMessage = 'Password for pfx in plaintext',
            Position = 1)]
        [String]$Password,
        [Parameter(Mandatory = $false,
    ...
    process {
        ...
        $x509Cert = [System.Security.Cryptography.X509Certificates.X509Certificate2]::new($secretByte,
            "", "Exportable,PersistKeySet") # Change from Microsoft Docs (se .NOTES). Couldn´t get method .import() to work so i used the constructor to achieve exact same result
        ...
        $pfxFileByte = $x509Cert.Export($type, $Password) # Generated bytearray encrypted with Password of type $type
